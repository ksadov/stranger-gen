...
const runWasm = async () => {
    ...
    var pixelsPtr = rustWasm.render_stranger();
    var pixelArray = new Uint8ClampedArray(rustWasm.memory.buffer, pixelsPtr, 4 * width * height);
	...
	pixelArray = new Uint8ClampedArray(rustWasm.memory.buffer, pixelsPtr, 4 * width * height);
	ctx.putImageData(new ImageData(pixelArray, width, height), 0, 0);
	}, false);
}    
